 ...
 * Gemini API 프록시. GEMINI_API_KEY는 Vercel 환경변수로만 관리.
 *
 * 보안 처리:
...
export default async function handler(req, res) {
  ...
  if (!apiKey) {
    console.error('[analyze] GEMINI_API_KEY 환경변수가 설정되지 않았습니다.');
...
점수 기준: 높을수록 양호. 자료에 명시적 근거가 없으면 중간값(55~65)으로 설정.`;
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      const msg = errData?.error?.message || `Gemini 오류 (${geminiRes.status})`;
      console.error('[analyze] Gemini API 오류:', msg);
